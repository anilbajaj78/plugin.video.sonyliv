...
# SafeString Used for Capitalization of Headers
...
def init_cookie_jar(addon_id):
    ...
    request = urllib2.Request("http://www.sonyliv.com")
    request.add_header('User-Agent', USER_AGENT)
    response = opener.open(request)

    TOKEN = response.info()["set-cookie"].split(",")[0].split(";")[0].split("=")[1]

    cookie_jar.save(cookie_file, ignore_discard=True)


    xbmc.log("Cookie :-")
    xbmc.log(cookie_file)
...
def add_dir(addon_handle, base_url, name, url, mode, icon_image='DefaultFolder.png', thumbnail='DefaultFolder.png', is_folder=True, background=None):
    u = base_url + '?' + urllib.urlencode({'url': urllib.quote(url, safe=''), 'mode': mode, 'name': urllib.quote(name.encode('utf-8'), safe='')})
    #xbmc.log("URL :" + str(u))
...
def add_dir_video(addon_handle, name, url, thumbnail, plot, duration):
...
def make_request(url, cookie_file, cookie_jar, TOKEN):
    opener = urllib2.build_opener(urllib2.HTTPCookieProcessor(cookie_jar))
    request = urllib2.Request(url)
    request.add_header('User-Agent', USER_AGENT)
    request.add_header(SafeString("XSRF-TOKEN"), TOKEN)
    response = opener.open(request)
    data = response.read()
    cookie_jar.save(cookie_file, ignore_discard=True)
...
def make_request_post(url, data, cookie_file, cookie_jar, TOKEN):
    ...
    request.add_header(SafeString("X-XSRF-TOKEN"), TOKEN)
